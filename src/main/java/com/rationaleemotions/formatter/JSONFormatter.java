...
class JSONFormatter implements Reporter, Formatter {

    private Map<CucumberTagStatement, Map<String, List<Map<String, Object>>>> mapping = new ConcurrentHashMap<>();
    private final NiceAppendable out;
    private String uri;

    private static final ThreadLocal<JsonDataContainer> data =
            new InheritableThreadLocal<JsonDataContainer>() {
                @Override
                protected JsonDataContainer initialValue() {
                    return new JsonDataContainer();
                }
            };


    private enum Phase { match, embedding, output, result}
    ...
    public void feature(Feature feature) {
        data.get().featureMap = feature.toMap();
        data.get().featureMap.put("uri", uri);
        data.get().featureMaps.add(data.get().featureMap);
    }
    ...
    public void scenario(Scenario scenario) {
        ...
        if (data.get().beforeHooks.size() > 0) {
            getFeatureElement().put("before", data.get().beforeHooks);
            data.get().beforeHooks = new ArrayList<>();
        }
    }
    ...
    public void before(Match match, Result result) {
        data.get().beforeHooks.add(buildHookMap(match,result));
    }

    @Override
    public void after(Match match, Result result) {
        List<Map> hooks = getFeatureElement().computeIfAbsent("after", k -> new ArrayList<>());
    ...
    @Override
    public void done() {
        for (Map<String, List<Map<String, Object>>> value : mapping.values()) {
            Map<String, Object> each = new HashMap<>();
            for (Map.Entry<String, List<Map<String, Object>>> eachValue: value.entrySet()) {
                each.put(eachValue.getKey(), eachValue.getValue());
                data.get().featureMaps.addAll(eachValue.getValue());
            }
        }
        out.append(gson().toJson(data.get().featureMaps));
    ...
    private List<Map<String, Object>> getFeatureElements() {
        CucumberTagStatement currentStatement = ScenarioDetailsManager.getCurrentScenarioDetails().getScenario();
        Map<String, List<Map<String, Object>>> localMapping = mapping.computeIfAbsent(currentStatement, key -> new HashMap<>());
        return localMapping.computeIfAbsent("elements", key -> new ArrayList<>());
    }
